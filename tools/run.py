...
"""
General usage:
./run.py <run-options> -- python script.py <script-options>
"""
...
PROJECT_NAME = open("./PROJECT_NAME").readlines()[0].strip()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=f"Run script wrapper to start {PROJECT_NAME} docker container.",
...
def get_relative_output_dir():
    ...
    return joinpath("/app", "results", PROJECT_NAME, output_dir_rel)
